...
public class ExprDefaultGroup extends SimpleExpression<Group> {

    static {
        Skript.registerExpression(
                ExprDefaultGroup.class,
                Group.class,
                ExpressionType.SIMPLE,
                "[the] realm['s] default group",
                "[the] default group of the realm"
        );
    }
    ...
    protected Group @Nullable [] get(Event event) {
        ...
        try {
            Group group = provider.defaultGroup();
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "the realm's default group";
    }
}