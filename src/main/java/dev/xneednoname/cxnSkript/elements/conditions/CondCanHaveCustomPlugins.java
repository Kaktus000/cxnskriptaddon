...
public class CondCanHaveCustomPlugins extends Condition {

    static {
        Skript.registerCondition(CondCanHaveCustomPlugins.class,
                "[the] [realm] (1¦can|2¦can('t| not)) have custom plugins");
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "whether the realm can have custom plugins or not";
    }
    ...
    public boolean check(Event event) {
        ...
        try {
            ...
            boolean canHaveCustomPlugins = limits.customPlugins();
            return isNegated() ? !canHaveCustomPlugins : canHaveCustomPlugins;
        } catch (Exception e) {
            Skript.warning("Failed to get realm custom plugin allowance: " + e.getMessage());
            return !isNegated();
        }
    }
}